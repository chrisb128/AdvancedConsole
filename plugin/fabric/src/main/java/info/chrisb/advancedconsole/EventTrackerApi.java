...
public class EventTrackerApi {
    ...
    public boolean setServerStatus(String status, Collection<Player> loggedInUsers) {
        if (!loginIfRequired()) return false;

        String userList = "[";
        Iterator<Player> iterator = loggedInUsers.iterator();
        boolean first = true;
    ...
    public boolean addEvent(EventType type, Player player, String message) {
        if (!loginIfRequired()) return false;
